       ...
       orderr?.map(order =>{
         return(
          ...
          <tr>
            ...
            <td>{order.home}</td>
            <td>{order.date}</td>
            <td className={order.status== 'Approve' ? 'text-success' : 'text-danger' }>{order.status}</td>
            <td> <button onClick={()=>cancelHandle(order._id)} className='border-0 bg-danger text-white fs-6'> Cancel </button></td>
           <td>  <button className='border-0 text-white bg-success fs-6' onClick={()=>handleAccept(order._id)}> Accept </button></td>
          </tr>
        </tbody>
         )
       })
